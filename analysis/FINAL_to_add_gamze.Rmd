...
Higher temperature has a positive trend with higher number of disasters in addition to  higher number of deaths and those affected by disasters.
```{r}
...
ggplot(molten.temp,aes(x=tmean,y=value,color=variable)) + 
  geom_smooth(method = lm, se = FALSE) +
...
```{r}
countryfinal2 <- merge(countryfinal,population_data,by.x='Country',by.y='Country')
ggplot(countryfinal2,aes(y=avg_gdp,x=sum_death)) + 
  geom_smooth(method = 'loess') + 
  geom_point() + 
  ggtitle("Number of Deaths from Disasters to Average GDP") +
  xlab("Number of Deaths from Disasters") + 
  ylab('Average GDP') +
  theme_minimal()  + xlim(0,25000) +ylim(0,3800000000000)

ggplot(countryfinal2,aes(y=avg_life_exp,x=sum_death)) + 
  geom_smooth(method = 'loess') + 
  geom_point() + 
  ggtitle("Number of Deaths from Disasters to Life Expectancy") +
  xlab("Number of Deaths from Disasters") + 
  ylab('Life Expectancy') +
  theme_minimal()  + xlim(0,26000) +ylim(45,90)
```
